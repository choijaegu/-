...
public class ServerGUIChatExam {
   ...
   public ServerGUIChatExam() 
   {
      try(ServerSocket server = new ServerSocket(9000))
      {
         while(true) 
         {	
        	System.out.println("Clinet 접속 대기중입니다~~~");
            sk=server.accept();
   ...
   /**
    *list에 있는 thread를 조회해서 pw로 데이터 전송하기 
    */
   public void sendMessage(String message)
   {
	   for(ClientSocketThread	th	:list	)
	   {
		   th.pw.println(message);						//플러쉬하려면 ln이나 f로 해야함
	   }
   }
   ...
   class ClientSocketThread extends Thread{
      ...
      public void run() 
      {
         ...
         catch(Exception e) 
         {
            ...
            System.out.println("["+nickName+"]님이 퇴장하셨습니다. | 현재 접속인원 = "+list.size()+"명");
